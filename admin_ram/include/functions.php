...
$sql=$GLOBALS['obj']->query("select * from tbl_content where id='$id' ");
$result=mysqli_fetch_object($sql);
	if($lang=='en'){
		return (stripslashes($result->content_en));
	}else if($lang=='ar'){
		return (stripslashes($result->content_ar));
	}else{
		return (stripslashes($result->content_en));
	}
}

function getTitle($id,$lang) {
$sql=$GLOBALS['obj']->query("select * from tbl_content where id='$id'",$debug=-1);
$result=mysqli_fetch_object($sql);
	if($lang=='en'){
		return (stripslashes($result->title_en));
	}else if($lang=='ar'){
		return (stripslashes($result->title_ar));
	}else{
		return (stripslashes($result->title_en));
	}
}
