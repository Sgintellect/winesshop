...
if(count($arr)>0){
	...
	{	  
	   ...
	   $sql="delete from $table_techer where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{	
		$sql="update $table_techer set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $table_techer set status=2 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $table_techer set status=3 where id in ($str_rest_refs)";
		$obj->query($sql);
		$sess_msg='Selected record(s) deactivated successfully';
		$_SESSION['sess_msg']=$sess_msg;
	}
	...
	{	
		...
		foreach($str_rest_refsArr as $val){
			$sql=$obj->query("insert into $table_techer_assign set emp_id='$emp_id',user_id='$val',assign_date='$assign_date',cdate='$cdate'",$debug=-1); //die;
		}
